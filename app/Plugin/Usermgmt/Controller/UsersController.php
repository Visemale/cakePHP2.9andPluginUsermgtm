...
class UsersController extends UserMgmtAppController {
	...
	public function index() {
            $this->set('title_for_layout', '<i class="fa fa-user"></i> Todos los Usuarios');
            $this->User->unbindModel( array('hasMany' => array('LoginToken')));
            $get_users_groups = $this->UserGroup->find('list');
            $users=$this->User->find('all', array('order'=>'User.id desc'));
            $this->set(compact('users','get_users_groups'));
	}
        
        public function buscar(){
            $this->view = 'index';
            if($this->request->is('post')){
                $data = $this->request->data['buscar'];
                if(empty($data) || $data == ''){
                    ...
                    $users = $this->User->find('all',array(
                                                    'conditions' => array(
                                                        'OR' => array(
                                                            'User.username LIKE ' => '%'.$data.'%',
                                                            'User.name LIKE ' => '%'.$data.'%',
                                                            'User.email LIKE ' => '%'.$data.'%'
                                                        )
                                                    ),
                                                    'order'	=> 'User.id DESC'
                                                ));

                    $this->set(compact('users','data', 'get_users_groups'));
                }
            }
	}
	...
	public function viewUser($userId=null) {
            $this->set('title_for_layout', '<i class="fa fa-user"></i> Detalle Usuario');
            if (!empty($userId)) {
	...
	public function myprofile() {
            $this->set('title_for_layout', '<i class="fa fa-coffee"></i> Mi Perfil');
...
//                die();
	...
	public function logout() {
		...
		$this->Session->setFlash(__('Ha Cerrado Sesión Exitosamente'));
		$this->redirect('/');
	}
	/**
			...
			if ($this->request -> isPost()) {
				...
				if ($this->User->RegisterValidate()) {
					...
					if (isset($this->request->data['User']['active']) && $this->request->data['User']['active']) {
						...
						$this->Session->setFlash(__('Por favor revise su correo y confirme su registro'));
	...
	public function changePassword() {
                $this->set('title_for_layout', '<i class="fa fa-key"></i> Cambiar Contraseña');
		$userId = $this->UserAuth->getUserId();
		if ($this->request -> isPost()) {
			$this->User->set($this->data);
			if ($this->User->RegisterValidate()) {
	...
	public function addUser() {
            ...
            $Breadcrums[] = array('Todos los Usuario','/allUsers',1);
            $Breadcrums[] = array('Nuevo Usuario','',2);
            $this->set('Breadcrums', $Breadcrums);
            ...
            if ($this->request -> isPost()) {
                ...
                if (isset($data) && count($data) > 0){  
                    ...
                    $data_users = $this->User->find('all', array('conditions' => array('User.email' => $data["User"]['email']), "limit" => 1));
                    if(count($data_users) > 0){
                        ...
                        $data['User']['password'] = $this->UserAuth->makePassword($data['inpConfirPass']);
	...
	public function editUser($userId=null) {
            $Breadcrums = array();
            $Breadcrums[] = array('Inicio','/user_dashboard',0);
            $Breadcrums[] = array('Listado Usuarios','/allUsers',1);
            $Breadcrums[] = array('Editar Usuario','',2);
            $this->set(compact('Breadcrums'));
            
            $conditions = array();
		if (!empty($userId)) {
                    $userGroups = $this->UserGroup->find('list');
                    $data_users = $this->User->find('all', array('conditions' => array('User.id' => $userId)));
                    ...
                    if ($this->request -> isPut()) {
                        ...
                        if ($this->User->RegisterValidate()) {
                                ...
                                $this->User->create();
                                $this->User->save($this->request->data,false);
                                $this->Session->setFlash(__('Los datos del usuario se actualizaron correctamente'));
                                $this->redirect('/allUsers');
                        }
                    } else {
                        $user = $this->User->read(null, $userId);
		...
		if (!empty($userId)) {
			if ($this->request -> isPost()) {
				if ($this->User->delete($userId, false)) {
					$this->Session->setFlash(__('El usuario fue eliminado con éxito'));
				}
			}
			$this->redirect('/allUsers');
		} else {
	 ...
	 * @return array
	 */
	public function dashboard() {            $this->layout = "default";
                $this->set('title_for_layout', '<i class="fa fa-home"></i> Inicio');
		$userId=$this->UserAuth->getUserId();
		$user = $this->User->findById($userId);
		$this->set('user', $user);
	}
	...
	public function forgotPassword() {
		...
		if ($this->request -> isPost()) {
			...
			if ($this->User->LoginValidate()) {
				...
				$this->Session->setFlash(__('Por favor, compruebe su correo para restablecer la contraseña'));
				$this->redirect('/login');
			}
		}
	}
